...
function GilTracker.Draw(event, tick)
    ...
    if (GUI:Begin("GilTrackerFixedBar###GilTrackerFixed", true, flags)) then
        ...
        if (not GilTracker.initialized) then
            ...
            elseif (d < 0) then 
                GUI:TextColored(1.0, 0.4, 0.4, 1.0, GilTracker.cache.change)
            else 
            ...
            if (isHovered and not GUI:IsPopupOpen("GilTrackerContextMenu")) then
                ...
                elseif (d < 0) then GUI:TextColored(1.0, 0.4, 0.4, 1.0, GilTracker.cache.change)
                ...
                GUI:Text("Total Expense:")  GUI:SameLine(colWidth) GUI:TextColored(1.0, 0.4, 0.4, 1.0, GilTracker.FormatNumber(GilTracker.stats.totalExpense))
                ...
                if (#GilTracker.history > 0) then
                    ...
                    for i = count, start, -1 do
                        ...
                        if (item.isStart) then
                            ...
                            local color = (delta > 0 and {0.1, 1.0, 0.1, 1.0}) or (delta < 0 and {1.0, 0.4, 0.4, 1.0}) or {1,1,1,1}
